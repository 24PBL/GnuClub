...
export default function MorePost() {
    ...
    const sections = {
      ...
      useEffect(() => {
        if (isFocused) {
          fetchPostInfo(); // 화면 활성화 시 데이터 가져오기
        }
    ...
    useEffect(() => {
        fetchPostInfo(); // 컴포넌트가 렌더링될 때 사용자 정보 가져오기
      }, []);
  ...
  >
    ...
    {posts.map((post, index) => (
      <TouchableOpacity key={post.postId} style={styles.clubBox} onPress={() => 
        navigation.navigate('Board', { clanId : post.clanId,
