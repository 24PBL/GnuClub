...
const { afterUploadImage, uploadPost, sendPostData, modifyImg, modifyPost, deletePost, modifyImage } = require('../controllers/post');
...
router.post('/:userId/:clanId/create-post/upload-image', isLoggedIn, (req, res, next) => {
...
router.post('/:userId/:clanId/create-post/upload-post', isLoggedIn, upload2.none(), uploadPost);

// 게시글 하나를 클릭했을 때 해당 게시글 페이지를 렌더링하기 위한 데이터들을 제공
// GET /post/:userId/:clanId/:postId
router.get('/:userId/:clanId/:postId', isLoggedIn, sendPostData);

// 게시글 수정 과정에서 이미지 수정
// PUT /post/:userId/:clanId/:postId/modify-image
router.put('/:userId/:clanId/:postId/modify-image', isLoggedIn, (req, res, next) => {
        upload.single('img')(req, res, (err) => {
            // 이미지 업로드 실패했을 때
            if (err) {
                console.error(err);
...
router.put('/:userId/:clanId/:postId/modify-post', isLoggedIn, upload2.none(), modifyPost);

// 포스팅을 삭제하는 라우터
// delete /post/:userId/:clanId/:postId/delete-post
router.delete('/:userId/:clanId/:postId/delete-post', isLoggedIn, deletePost);
